...
@section('content')
    <div class="col-md-12">
        <h1>Şahsy düzümiň hasaby </h1>
        <hr class="hr-header">
    </div>

    <div class="row my-3 mx-1">
        <div class="col-md-12 text-right">
            <form action="{{route('expenditure')}}" method="GET">
                ...
                <a href="{{route('serviceManInfo')}}" class="btn bg-navbar float-left m-1"> <i class="fa fa-info"> </i> Umumy maglumat </a>
            </form>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <ul class="list-group">
                <li class="list-group-item d-flex justify-content-between align-items-center bg-navbar text-light">
                    Talyplaryň taýýarlyk we okuw sapaklaryna gatnaşygynyň hasaby.
                </li>
                <br>
                <div class="ml-2 mr-2">
                    <table id="group_report_week" class="table table-hover table-bordered text-center m-md-1" style="width:100%">
                        ...
                        @foreach($groupReportForWeek as $group)
                            <tr>
                                <td>{{$group->number}}</td>
                                <td style="width: 12%;">{{$group->kathedra->name}}</td>
                                <td class="pr-4">
                                ...
                                <td class="pr-4">
                                    <h5><a href="{{route('open.expenditure',['para'=>$para, 'date'=>$date ,'group_id'=>$group->id])}}"> <span class="badge badge-success badge-pill">Gormek</span></a></h5>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
